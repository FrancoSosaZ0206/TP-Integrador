...
FechaPtr cargarFecha()
{
    ...
    do{
        ...
        else if(!quedaTiempo(fecha))
        {
            printf("\n\nERROR: La fecha ingresada ya paso.");
            presionarEnterYLimpiarPantalla();
        }
    } while (!esFechaValida(fecha) || !quedaTiempo(fecha));
    return fecha;
}
    ...
    /// //////////////////////////
        ...
        do
        {
            ...
            case 1:
                ...
                printf("\n\n");
                mostrarReparto((RepartoPtr)getDatoLista(listaAux,i-1));
                presionarEnterYLimpiarPantalla();
                break;
            case 2:
                do
                {
                    ...
                    case 1:
                        primeraVez=true;
                        do
                        { //Primero, ingresamos y validamos la fecha
                            printf("MOSTRAR LISTA DE REPARTOS FILTRADOS POR FECHA DE SALIDA\n\n");

                            printf("Ingrese una fecha para filtrar [DD MM AAAA]: ");
                            scanf("%d %d %d",&dia,&mes,&anio);
                            limpiarBufferTeclado();

                            if(primeraVez)
                                fecha=crearFecha(dia,mes,anio,0,0);
                            else
                            {
                                setDia(fecha,dia);
                                setMes(fecha,mes);
            ...
            case 3:
                do
                {
                    ...
                    case 0:
                        break;
                    case -1:
                        op2=0; //En lugar de poner cada una en 0, ponemos en -1 para que vaya a la cláusula del menú anterior, haciendo todo de una
                        op3=0;
                        *opMenuAnterior=0;
                        break;
                    default:
...
//------------------------------------------//
    ...
    //------------------------------------------//
        ...
        /// Ahora, cargamos y validamos la fecha de retorno
            int hora=0,minuto=0;
...
bool menuActualizarReparto(CentroLogisticoPtr centroLogistico)
{
    ...
    else
    {
        ...
        else
        {
            ...
            int total = 0;
            for(total=0;!pilaVacia(getPaquetesReparto(repartoActualizar));total++)
                paquetesModificar[total] = descargarPaquete(repartoActualizar);
            ...
            else
            {
                ...
                for(int i = total-1 ; i > -1 ; i--)
                    cargarPaquete(repartoActualizar, paquetesModificar[i]);
            }
        }
        return false;
    }
